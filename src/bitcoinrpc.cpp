...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "grumpycoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "grumpycoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // grumpycoin: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
