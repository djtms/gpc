 ...
 * The grumpycoin Developers 2013
 */
...
{
    resize(850, 550);
    setWindowTitle(tr("grumpycoin") + "" + tr(""));
#ifndef Q_WS_MAC
...
void BitcoinGUI::setMining(bool mining, int hashrate)
{
    if (mining)
    {
        ...
        labelMiningIcon->setToolTip(tr("Mining grumpycoins at %1 hashes per second").arg(hashrate));
    }
    else
    {
        labelMiningIcon->setPixmap(QIcon(":/icons/mining_inactive").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
        labelMiningIcon->setToolTip(tr("Not mining grumpycoins"));
    }
}
